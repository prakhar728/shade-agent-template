...
import { NearModule } from "../utils/near";
...
type UsdTokensDecimals = Record<string, number>;

const usdTokensDecimals: UsdTokensDecimals =
...
type UsdTokens = Record<string, any>;

const usdTokens: UsdTokens = Object.entries(usdTokensDecimals).reduce(
  (acc: UsdTokens, [key, value]: [string, number]) => {
...
interface RefFinance {
  ...
  poolsByToken: Record<string, Pool[]>;
  poolsByPair: Record<string, Pool[]>;
}
...
interface SwapInfo {
  ...
  expectedAmountOut?: any;
}

function stablePoolGetReturn(
  pool: Pool,
  tokenIn: string,
  amountIn: any,
  tokenOut: string,
  ...
  // Sub 1
  const cAmountIn = (amountIn as any)
    .sub(1)
    .mul(Big(10).pow(18 - (usdTokensDecimals[tokenIn] as number)));
...
function stablePoolGetInverseReturn(
  ...
  const amountOutWithFee = amountOut
    ...
    .round(0, 0);
  const cAmountOut = (amountOutWithFee as any).mul(
    Big(10).pow(18 - (usdTokensDecimals[tokenOut] as number))
  );
  ...
  return (cAmountIn as any)
    .div(Big(10).pow(18 - (usdTokensDecimals[tokenIn] as number)))
    .add(1)
    .round(0, 0);
}

function getRefReturn(
  pool: Pool,
  tokenIn: string,
  amountIn: any,
  tokenOut: string,
...
function stablePoolComputeD(pool: Pool): any {
  let sumX = pool.cAmounts!.reduce((sum: any, v: any) => sum.add(v), Big(0));
  if (sumX.eq(0)) {
    return Big(0);
  } else {
    let d = sumX;
    ...
    for (let i = 0; i < 256; ++i) {
      ...
      for (let j = 0; j < pool.nCoins!; ++j) {
        dProd = dProd
          .mul(d)
          .div((pool.cAmounts![j] as any).mul(pool.nCoins!))
          .round(0, 0);
      }
      dPrev = d;

      let leverage = (sumX as any).mul(pool.ann!);
      let numerator = (dPrev as any).mul((dProd as any).mul(pool.nCoins!).add(leverage));
      let denominator = (dPrev as any)
        .mul((pool.ann! as any).sub(1))
        .add((dProd as any).mul(pool.nCoins! + 1));
...
function stablePoolComputeY(
  ...
  let d: any = pool.d!;
  let s: any = xCAmount;
  let c: any = d.mul(d).div(xCAmount).round(0, 0);
  pool.cAmounts!.forEach((c_amount: any, idx: number) => {
...
async function prepareRef(
  ...
  const promises = poolIds.map((pool_id: number) =>
    near.NearView(nearConfig.refContractName!, "get_pool", { pool_id })
  );
  const rawPools = await Promise.all(promises) as RawPool[];

  const poolsByToken: Record<string, Pool[]> = {};
  const poolsByPair: Record<string, Pool[]> = {};

  const addPools = (token: string, pool: Pool) => {
    let ps = poolsByToken[token] || [];
    ps.push(pool);
    poolsByToken[token] = ps;

    pool.ots[token].forEach((ot: string) => {
      const pair = `${token}:${ot}`;
      ps = poolsByPair[pair] || [];
      ps.push(pool);
      poolsByPair[pair] = ps;
  ...
  rawPools.forEach((pool: RawPool, i: number) => {
    if (pool.pool_kind === SimplePool || pool.pool_kind === StablePool) {
      ...
      const p: Pool = {
        ...
        index: i,
        tt,
        tokens: tt.reduce((acc: Record<string, any>, token: string, tokenIndex: number) => {
          acc[token] = Big(pool.amounts[tokenIndex]);
          return acc;
        }, {}),
        ots: tt.reduce((acc: Record<string, string[]>, token: string) => {
          acc[token] = tt.filter((t: string) => t !== token);
          return acc;
        }, {}),
        fee: pool.total_fee,
      ...
      if (p.stable) {
        p.cAmounts = [...pool.amounts].map((amount: string, idx: number) => {
        ...
        p.ann = Big(p.amp ?? 0).mul(p.nn);
        p.d = stablePoolComputeD(p);
      }

      if (p.shares.gt(0)) {
        pools[p.index] = p;
        p.tt.forEach((t: string) => addPools(t, p));
      }
    }
  });
  ...
  Object.values(refFinance.poolsByToken[inTokenAccountId] || {}).forEach(
    (pool: Pool) => {
      ...
      if (outTokenAccountId in pool.tokens) {
        ...
        // 2 tokens
        pool.ots[inTokenAccountId].forEach((middleTokenAccountId: string) => {
          const pair = `${middleTokenAccountId}:${outTokenAccountId}`;
          let poolReturn: any = false;
          Object.values(refFinance.poolsByPair[pair] || {}).forEach(
            (pool2: Pool) => {
              poolReturn =
                poolReturn === false
                  ? getRefReturn(
  ...
  Object.values(refFinance.poolsByToken[outTokenAccountId] || {}).forEach(
    (pool: Pool) => {
      ...
      if (inTokenAccountId in pool.tokens) {
        ...
        // 2 tokens
        pool.ots[outTokenAccountId].forEach((middleTokenAccountId: string) => {
          const pair = `${middleTokenAccountId}:${inTokenAccountId}`;
          let middleAmountIn: any = false;
          Object.values(refFinance.poolsByPair[pair] || {}).forEach(
            (pool2: Pool) => {
              middleAmountIn =
...
export default async function computeUsnPriceMultiplier(
  near: NearModule,
  usnTokenId: string,
  usdtTokenId: string,
  stablePoolId: number
  ...
  if (usnPriceMultiplier < 0.95 || usnPriceMultiplier > 1.05) {
    console.error(
      `USN stable pool is unbalanced. Price: ${usnPriceMultiplier}`
    );
    return null;
  }
